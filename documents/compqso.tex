...
The data were reduced using the ESO/X-shooter pipeline v2.5.2 (Modigliano), where for simplicity we have rectified all spectra on a grid with 0.2 \AA/pixel thus slightly oversampling even the highest-resolution spectra without introducing too much correlation between adjacent pixels in the rectification. The spectra were extracted in the standard way and were flux calibrated using photometric standards. Telluric observations of hot dwarfs were taken close in time to allow for an accurate telluric correction which depends heavily on the atmospheric conditions at the time of observation.
