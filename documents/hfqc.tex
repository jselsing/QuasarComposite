...
\abstract{
QSO spectral templates are important to both examine QSO physics and for investigations that use QSOs as probes of intervening gas and dust. However, combinations of various QSO samples obtained at different times and with different instruments to expand a composite to a larger rest frame wavelength region may create systematic effects, and the contribution from QSO hosts may contaminate the composites. Here we examine a composite spectrum constructed from luminous blue QSOs at 1< z < 2.3 selected from the SDSS.  The observations with X-shooter cover simultaneously UV to near-IR  wavelength ranges, which ensures that the composite spectrum covers the full rest-frame range from Lybeta to 8500 A without any significant host contamination. Assuming a power law continuum for the composite we find a spectral slope of  $\alpha$  = 1.71$\pm$0.09, which is steeper than previously found in the literature. We attribute the differences to the fact that we have a larger spectral wavelength coverage, which allows us to effectively avoid fitting regions affected either by strong QSO emissions lines (e.g. balmer lines, and complex [Fe II] blends) or by intrinsic host galaxy emission. Finally, we demonstrate the use of the dust-free QSO composite spectrum to evaluate the reddening in other QSOs.  (which is useful for….????? etc.)
...
%Observations of hot B-type dwarf stars taken close in time were used to make a correction for telluric absorption lines, which depend heavily on the atmospheric conditions at the time of observation.
...
We show the weighted mean composite in \fig{combined}. Characteristic features of quasars are readily visible. We see several prominent emission lines across the entire spectral range which includes both very broad high-ionization lines and narrower low-ionization lines as is typical for quasar spectra, with both broad and narrow lines superposed with multiple components \citep{Baldwin1995}. We have marked the position and name of the most prominent lines and overplotted them in \fig{combined}.  Analysis of the lines are beyond the scope of this paper.
 ...
 \begin{figure*}[t!]
   ...
   \textit{Lower middle panel:} Signal-to-noise ratio. The composite spectrum is divided by the error spectrum, thus directly giving a measure of the signal strength.
