...
\abstract{Quasi-steller object (QSO) spectral templates are important both to QSO
  ...
  to expand a composite to cover a larger rest frame wavelength region may
  create systematic effects and the contribution from QSO hosts may
  contaminate the composite. Here we construct a composite spectrum 
  from luminous blue QSOs at $1< z < 2.1$ selected from
  the Sloan Digital Sky Survey (SDSS).  The observations with X-shooter cover simultaneously ultra violet (UV)
  ...
  {\AA} without any significant host contamination. Assuming a power
  law continuum for the composite we find a spectral slope of \textbf{$\alpha_\lambda$
  = 1.72$\pm$0.01}, which is steeper than previously found in the
  literature. We attribute the differences to the fact that we have a
  larger spectral wavelength coverage, which allows us to effectively
  avoid fitting regions affected either by strong QSO emissions lines
  ...
  dust-free QSO composite spectrum to evaluate the reddening in other
  QSOs. }
...
\textbf{In the central southern SDSS footprint, Stripe 82 \citep{Annis2014}, about 50 such candidate QSOs has been studied using the
...
suffice. \textbf{From the 166,583 quasars presented in the SDSS DR10 quasar catalog \citep{Paris2014}, 173 objects fulfill our selection criteria. Of these quasars, 102 are at declinations < +15 from which we select 7 without signs of BAL activity or other
strong associated absorption systems at declinations $\lesssim +15^\circ$ that have SDSS spectra and are
observable in a single visitor run at the Very Large Telescope (VLT).} With this sample, all restframe
...
\sect{systematics}.
%Observations of hot B-type dwarf stars taken close in time were used to make a
%correction for telluric absorption lines, which depend heavily on the
...
slightly blended with [\ion{O}{iii}]$\lambda$4959 and \hb, with \textbf{\hb~containing both broad} and narrow components.  
...
in a 3 arcmin radius around the source are extracted from the map yielding four pixels from which an arithmetic average is taken}. The reddening law by
...
through a filter where if the change between neighbouring pixels is greater
than 10 percent, the pixel is flagged as a bad and excluded from the
combination. Consequtive comparison is carried out between pixels in order of ascending wavelength. This filtering algothm is applied to exclude noisy pixels from the pipeline and pixels affected too
...
wavelength. To normalize for differing slopes in between the different spectra,
the fitted slope in each of the constituent spectra is divided by the standard
deviation. 
...
$\alpha_\lambda = -1.72$, where for a broken power law, we obtain a spectral
index $\alpha_\lambda = -1.73$ below 5700 \AA, and $\alpha_\lambda = -1.72$
...
\textbf{There exists a distribution of slopes for quasar continua and when using the composite presented here, the intrinsic slope of the modelled quasar is highly degenerate with the amount of dust inferred \citep{Reichard2003a}. Since attenuation by dust is stronger at shorter wavelengths, extinction introduces a curvature in the spectrum which is seperate from a pure change in slope, see
\citet{Krawczyk2015} for a discussion of \textit{red vs. reddened}. }
...
% but the reader is cautioned that only three spectra contribute significantly
...
%%We have to be careful with regards to the conclusions and the sample
%selection. We have selected bright quasars, but we can only say something about
%the mean of this particular sample and not something about the general
%population of quasars. If there exists an intrinsic population of quasars who
%are redder, this effect will be degenerate with dust, and fitting for the dust
%are not guaranteed to result in robust results. This is of major concern.  
%\qtodo{Talk to Jens about this again}.
%
%Something extremely profound.
%
%Due to the intrinsic brightness of our objects, we expect the black-hole mass
%to be high, compared lower-brightness objects. \citet{Lusso2015} section 5.5
%find that the break in power-law slope at$ \sim 920$ \AA is which does not
%agree with Shakura sunyaev prediction. They need high black-hole spin or wrong
%black-hole masses to make the two models fit.  
%If a model SED for a quasar is needed a shallower slope than the one from
%\citep{VandenBerk2001}, is found, 
...
contamination}.
 ...
 \begin{figure*}[t!]
   ...
   \caption[]{Comparison of different composites. The composites by
     ...
     with slope $\alpha_\lambda = -1.72$ and normalized at $\sim 1450$ \AA.}
...
different wavelength coverages. \textbf{A small region free of line-emission just redwards of \lya~is covered by most of the composites in the comparison, but fitting a power-law to such a small region would largely leave the inferred slope dependant on the exact ranges chosen for the fit.}
...
\input{tabs/combinations.tex} The systematic error in the reported
...
deviations of the constituent spectra at each spectral bin \textbf{divided by the flux}, and where
...
regions.
%Composites constructed from the arithmetic mean whose constituent spectra are
%normalized in the blue are expected to have shallower slopes and therefore
%appear redder and vice versa for normalization in the red, because the relative
%weighting of the different regions changes as a function of normalization
%region. To quantify this effect we have generated mock quasar spectra from
%broken power laws with varying slopes and normalizations and constructed the
%arithmetic mean composites where the constituent spectra have been normalized
...
%   \caption[]{Composites generated from different normalization regions .
%\jstodo{First draft for figure. Re-iterate}}
%  \figlabel{mean_test}
% \end{figure}
% %===========================================================================
% A clear change in slope is detected with red normalization yielding bluer
%composites. Here we have scaled the line-strengths with the normalization so
%the luminosity of the mock spectra do not affect line-strengths and we also see
%a change in the line strength relative to the continuum level, with the steeper
%slopes yielding more prominent lines. We do not see a steepening of slopes with
%normalization region to a significant degree in the real quasar composite and
%this supports the use of the weighted mean.
...
%%====================================================================
%%Sampling size
%To test the effect of the sampling step size chosen we do the construction of
%the with varying sampling steps and we find no difference in the qualitative
%results, with variations mainly consisting of a higher degree of smoothness
%when going to larger sampling steps, since small scales are being resolved out
%and more pixels without increased resolution when going down in sampling step
%size. 
...
power law continuum we find a spectral slope of $\alpha_\lambda = -1.72 \pm 0.01$, a
...
\input{tabs/example.tex} 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Acknowledgements}  \sectlabel{Acknowledgements}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{We thank the anonymous referee for constructive feedback which has helped improve the strength of the work.}
...
%\newcommand*{\doi}[1]{\href{http://dx.doi.org/\detokenize{#1}
%{\raggedright\mybibdoicolor{DOI: \detokenize{#1}}}}
%\bibliography{references}
\bibliography{/Users/jselsing/Work/Papers/bibtex/XSHQuasarComposite}
...
%%%===========================================================================
%\section{Normalized magnitde at a given redshift}  \sectlabel{mathmag} \label{mathmag}
%%===========================================================================
%
%We here show the consequences of normalizing to a different redshift in the magnitude system and how to transform between the different systems. This is following the prescription of \citet{Richards2006b}.
%
%We start with the definition of the synthetic magnitude from \sect{absmag}
%
%\begin{eqnarray}\eqlabel{abmagderiv1}
%m_{\textmd{AB}_x} &=& -2.5 \log_{10} \left(  \frac{\int f_\lambda \,  T_{x}  \,
%\lambda \,  d\lambda} 
%{\int  T_{x} \,  (c / \lambda) \,  d\lambda} \right) - 48.60,
%\end{eqnarray}
%We make the following transformation 
%\begin{eqnarray}\eqlabel{abmagderiv2}
%f_{\lambda} \rightarrow \frac{f_{\lambda (1 + z)}}{1 + z}  , \qquad \lambda \rightarrow \lambda \cdot (1 + z)
%\end{eqnarray}
%This corresponds to artificially shifting the spectrum by a factor (1 + z), while conserving flux. This transformation yields the following expression for the apparent magnitude:
%
%\begin{eqnarray}\eqlabel{abmagderiv2}
%\begin{split}
%m_{\textmd{AB}_x}(z) &=& -2.5 \log_{10} \left(  \frac{\int\frac{f_{\lambda (1 + z)}}{1 + z}  \,  T_{x}  \,
%\lambda(1+z) \,  d\lambda} 
%{\int  T_{x} \,  (c / \lambda(1+z)) \,  d\lambda} \right) - 48.60 \\
%& = & -2.5 \log_{10} \left(  \frac{\int f_{\lambda (1 + z)}  \,  T_{x}  \,
%\lambda \,  d\lambda} 
%{\int  T_{x} \,  (c / \lambda) \,  d\lambda} \right) - 48.60 - 2.5 \log_{10} (1 + z)
%\end{split}
%\end{eqnarray}
%
%where by modeling the flux as a power-law, $f_{\lambda} =  k \lambda ^{ \alpha_{i} }$, allows us to make the following substitution: 
%
%
%\begin{eqnarray}\eqlabel{abmagderiv2}
%m_{\textmd{AB}_x}(z) &=& -2.5 \log_{10} \left(  \frac{\int\frac{f_{\lambda (1 + z)}}{1 + z}  \,  T_{x}  \,
