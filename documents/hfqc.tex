...
Figure 1 in \cite{Shen2011} presents the distribution of M$_i(z=2)$ as a function redshift for quasars spectroscopically confirmed from the SDSS, for which the black dots represents about half of the objects and are uniformly selected to $\sim$90 percent completeness \citet{Richards2002, VandenBerk2005}. It is clear that our sample represents some of the brightest quasar between redshift 1 and 2. It is therefore potentially a biased subset of the parent quasar population. Quasars have previously been shown to exhibit a large degree of homogeneity over many orders of magnitude \citep{Dietrich2002}, but this is only true for quasars resembling \textit{average} quasars. To ensure that the targets selected for this sample are not outliers, the photometric colors are compared to those of the quasar population presented in \citet{Paris2014}\footnote{\url{https://www.sdss3.org/dr10/algorithms/qso_catalog.php}}.
We see
 ...
 \begin{figure}[hbtp]
   \centering
   \includegraphics[width=0.99\columnwidth]{Figs/color_comparison.pdf}
   \caption[]{Quasar color, g-i,  as a function of absolute i-band magnitude at redshift two, M$_i$(z=2). The full quasar sample from SDSS DR10\citep{Paris2014} is histogrammed with grey colors in 200 bins. Bounding contours in green show the concentration of points. The quasar color have been marginalized over in the right side. The blue points represent the sample defined here and appear bluer than the general population.}
